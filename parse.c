...
int numOfPipes; //This global variable will be used in shell.c
...
/* -----------------------------------------------------------------------------
parse_command()
DESCRIPTION: This breaks up the command line argument and sorts them into the
appropriate linked lists in the commandType list.
-------------------------------------------------------------------------------*/
...
/* -----------------------------------------------------------------------------
print_info()
DESCRIPTION:  Prints info to the terminal, providing valuable information from
the info pointer.
-------------------------------------------------------------------------------*/
void print_info (parseInfo *info) 
{
  ...
  if (NULL == info) //If the info is null
  {
    	printf("Null info\n");
    	return;
  }
  ...
  /* Edited */numOfPipes = info->pipeNum; //Adds this to the global variable

  for( i=0; i<=info->pipeNum;i++ ) // Checks for number of pipes
  {
...
/* -----------------------------------------------------------------------------
free_info()
DESCRIPTION:  Frees the information from the previous command so that it can
be ready to accept new info from the new input on the command line.
-------------------------------------------------------------------------------*/
